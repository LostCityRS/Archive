...
export default class Js5LocalDiskCacheWrite {
    ...
    constructor(dir: string, archives: number) {
        ...
        fs.writeFileSync(`${dir}/main_file_cache.dat2`, new Uint8Array());
        this.dat = new RandomAccessFile(`${dir}/main_file_cache.dat2`);

        fs.writeFileSync(`${dir}/main_file_cache.idx255`, new Uint8Array());
    ...
    write(archive: number, file: number, src: Uint8Array, version: number) {
        if (archive !== 0) {
            // todo: modern caches may use 4 bytes for the version
